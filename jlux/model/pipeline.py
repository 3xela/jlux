...
class FluxPipeline:
    ...
    def sample(self, pooled, seq, B, height, width, num_steps, guidance, key):
        ...
        latent = euler_sample(model = self.batched_dit, img_ids = img_ids, x_init= latent, txt = seq, txt_ids=txt_ids, y = pooled, guidance = guidance, timesteps = timesteps)
    ...
    def __call__(self, prompts : list[str], height, width, num_steps, guidance, key) -> jnp.ndarray:
        ...
        latent = self.sample(pooled, seq, B = len(prompts), height = height, width = width, num_steps=num_steps, guidance = guidance, key = key)
